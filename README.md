...
- 构建一个**可复现的恶意代码分析实验平台**，覆盖从样本准备、规则管理到检测评估的完整流程  
- 帮助同学在工程实践中理解 **特征匹配类检测（YARA）** 与 **行为规则类检测（Sigma）** 的优缺点与适用场景  

系统整体由三个部分组成：

- **文件静态检测子系统（backend + tools）**  
  - 利用大规模 YARA 规则库与 RDS 白名单数据库，对 PE 文件进行特征匹配与打分  
  - 提供命令行批量扫描工具 `tools/comprehensive_scan.py`，可对良性/恶意样本集计算检出率、误报率等指标  
- **动态行为检测子系统（Sigma 引擎）**  
  - 自研 `SigmaEngine` 内核，从 `data/sigma_rules` 加载数千条 Sigma 规则，构建基于 EventID 的倒排索引  
  - 通过 `/api/sigma-scan/*` 系列接口，对 Windows 事件日志、VirusTotal 沙箱行为、模拟沙箱事件进行统一检测，输出规则命中与威胁等级  
- **可视化前后端管理平台（FastAPI + React/Vite）**  
  - 后端基于 FastAPI 提供 RESTful API，前端基于 React/Vite 实现扫描任务、规则列表、检测报告等页面  
  - 支持 Sigma 规则的在线浏览、上传与自定义规则管理，便于课程实验中的规则调试与扩展  

本工程为 **南开大学 2025 年秋季《恶意代码分析与防治技术》课程期末综合项目**，由以下同学共同完成：

- 郭子涵
- 巩岱松
- 肖俊涛  
- 陈展
- 均来自于**南开大学密码与网络空间安全学院**
...
│  └── course_rules/    # 存放课程 YARA 规则文件 (.yar, .yara, .yal)

├── sample/

│  └── BinaryCollection/  # 存放待检测的实验样本文件 (.exe, .dll, .sys 等)
...
在项目根目录下打开终端，运行以下命令，即可对教材样本和五个额外真实的恶意样本进行检测：
